...
class App extends Component{
  ...
  render(){
    ...
    const searchList=historyList.filter(each=>each.title.toLowerCase().startsWith(searchInput.toLowerCase()))
    return(
        ...
        <div className="header-container">
          ...
          <div className="input-container">
              ...
              <input type="search" placeholder="search history"
               className="input-text" value={searchInput} onChange={this.onChangeSearchInput} />
          </div>
        </div>
        <ul className="history-container">
            {searchList.length===0?<EmptyHistory/>:(<>{searchList.map(each=>(
              <HistoryItem historyDetails={each} key={each.id} updatehistoryList={this.updatehistoryList} />
            ))}</>)}           
        </ul>
      </div>
    )
